...
function topSelling(myAnalytics) {
  const products = [];
...
function drawChart(chartData) {
  if (chartData) {
    ...
    const topSells = topSelling(chartData.analytics);
    ...
    const salesGraph = canvas4.getContext("2d");

    // canvas width
    canvas.width = 400;
    canvas2.width = 400;
    canvas3.width = 400;
    canvas4.width = window.innerWidth * 0.8;
    // canvas height
    canvas.height = 300;
    canvas2.height = 300;
    canvas3.height = 300;
    ...
    // draw the charts
    const myBarChart = new Chart(salesChart).Bar(data, options);
    const myPieChart = new Chart(topSeller).Pie(pieData);
    const myLineChart = new Chart(topEarner).Doughnut(earnersData);
    const mySalesChart = new Chart(salesGraph).Line(data2, options);
  }
}
/**
